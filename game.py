...
def normalise_input(user_input):
    ...
    'help'
    """
    temp_str1 = remove_spaces(user_input)
    temp_str2 = remove_punct(temp_str1)
    temp_str3 = remove_spaces(temp_str2)
    temp_str4 = remove_punct(temp_str3)
    return temp_str4.lower()
...
def menu(exits):
    ...
    while True:
        ...
        user_input = input("> ")
        # Normalise the input
        norm_user_input = normalise_input(user_input)
        # Check if the input makes sense (is valid exit)
            # If so, return the player's choice
        if is_valid_exit(exits, norm_user_input):
            return norm_user_input
